...
async function displayPokemon(pokemonArray) {
  const pokemonInfo = pokemonArray
    .map(
      (pokemonArray) => `
            <li class="card" style="padding: 2%;margin: 2%;list-style-type: none;">
            <img class="card-image" src="${pokemonArray.image}" data-tilt/>
            <h2 class="card-title"> ${
              pokemonArray.name.charAt(0).toUpperCase() +
              pokemonArray.name.slice(1)
            }</h2>
            <p class="card-subtitle">Type: ${
              pokemonArray.type.charAt(0).toUpperCase() +
              pokemonArray.type.slice(1)
            }</p>
        </li>
      `
    )
